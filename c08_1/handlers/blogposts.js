...
const save = async (req, res) => { // save ne mi e jasen :/
    try {
        req.body.user_id = req.user.uid;
        await validator.validate(req.body, validator.blogpostSchema); // req.body mi e Basic Validation (vo Postman podatocite sto gi vnesuvam ), validator.blogpostSchema mi e Data Format Validation(required sto gi imam staveno da mi se, i treba da bidat isti za da se validiraat za da se zacuva nov korisnik)
